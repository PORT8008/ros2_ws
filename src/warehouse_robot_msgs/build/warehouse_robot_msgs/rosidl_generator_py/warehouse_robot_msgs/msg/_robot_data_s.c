...
bool warehouse_robot_msgs__msg__robot_data__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // ar_tag_id
    PyObject * field = PyObject_GetAttrString(_pymsg, "ar_tag_id");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->ar_tag_id = (int32_t)PyLong_AsLong(field);
    Py_DECREF(field);
  }
...
PyObject * warehouse_robot_msgs__msg__robot_data__convert_to_py(void * raw_ros_message)
{
  ...
  {  // ar_tag_id
    PyObject * field = NULL;
    field = PyLong_FromLong(ros_message->ar_tag_id);
    {
      int rc = PyObject_SetAttrString(_pymessage, "ar_tag_id", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
